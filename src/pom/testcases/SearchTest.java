...
public class SearchTest {
	...
	public void searchItems() throws Exception {
		...
		Thread.sleep(2000);
		boolean rFlag=searchpage.verifySearchInput("agave white rhino");
		Assert.assertTrue(rFlag);
		
		//String reFlag=searchpage.verifySearchResults();
		String sFlag=searchpage.verifySearchResults();
		Assert.assertEquals(sFlag, "Search Item not displayed");
		
		//Assert.assertEquals(reFlag, reFlag);
		Assert.assertTrue(sFlag.isEmpty(),sFlag);
		
		boolean aflag= searchpage.addToCart();
		Assert.assertTrue(aflag, "items are not added to the cart");
